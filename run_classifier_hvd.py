...
flags.DEFINE_bool("freeze", False, "Whether to freeze transferred hidden layers.")
...
class MetaProcessor(DataProcessor):
  """Processor for the metacritic data set."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
      if len(line) != 2:  # some rows don't read 'senti'
        print("couldn't read label on row %d!"%i)
        print(line)
        continue
      guid = "%s-%s" % (set_type, i)      
      text_a = tokenization.convert_to_unicode(line[0])
      ...
      # for align text with predicted labels
      if set_type == "test":
        with open('real_test.tsv','w') as f:
          newline = '\t'.join([text_a, tokenization.convert_to_unicode(line[1])])
          f.write(newline+'\n')
...
class RedditProcessor(DataProcessor):
  """Processor for the Reddit data set.
     Note that Reddit has no labels"""
  ...
  def _create_examples(self, lines, set_type):
    ...
    examples = []
    if set_type == "test":
        f = open('real_test.tsv','w')
    
    for (i, line) in enumerate(lines):
      # if i == 0:
      #   continue  # reddit does not have header
